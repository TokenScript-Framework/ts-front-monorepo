        ...
        <section className="min-h-screen fancy-overlay pt-4">
            <div className="container-wide mx-auto grid grid-cols-3 gap-8" key={type}>
                ...
                {tokenList.map((token, index) => (
                    ...
                    <Card className="text-center p-8 dark:bg-gray-900 cursor-pointer" onClick={() => loadNFTHandler(token.address, token.tokenId)} key={`${type}-c` + index}>
                        <CardTitle >
                        ...
                        <CardContent>
                            {type === 'ERC20' ? (
                                <>
                                    <div className="flex justify-between text-gray-500 dark:text-[#B3B3B3]" key={`${type}-20` + index}>

                                        <div>ChainId</div>
                                        <div>Symbol</div>
                                    </div>
                                    <div className="flex justify-between font-bold" key={`${type}-label` + index}>
                                        <div>{token.chainId}</div>
                                        <div>{token.symbol}</div>
                                    </div><br />
                                    <div className="flex justify-between opacity-7 text-gray-500 dark:text-[#B3B3B3]" key={`${type}-title` + index}>

                                        <div>Balance</div>
                                        <div>Decimals</div>
                                    </div>
                                    <div className="flex justify-between font-bold" key={`${type}-balance` + index}>
