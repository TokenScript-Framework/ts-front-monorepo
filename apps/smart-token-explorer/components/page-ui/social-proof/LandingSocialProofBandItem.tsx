  ...
  return (
    <div className={clsx("flex items-center justify-start gap-0.5", className)}>
      {graphic ? (
        <>
          {graphic === "checkmark" ? (
            <CheckCheckIcon className="mr-1.5 h-4 w-4 text-green-500" />
          ) : null}
          {graphic === "magic" ? (
            <Wand2Icon className="mr-1.5 h-4 w-4 text-yellow-500" />
          ) : null}

          {graphic === "trophy" ? (
            <TrophyIcon className="mr-1.5 h-4 w-4 text-yellow-500" />
          ) : null}

          {graphic === "gift" ? (
            <GiftIcon className="mr-1.5 h-4 w-4 text-green-500" />
          ) : null}
