...
import { chainPipe, getTokenscriptMetadata, networkPipe, rewriteUrlIfIFPSUrl } from "./utils";
...
export async function validateToken(
  ...
  let scriptMetadata;
  try {
    scriptMetadata = await getTokenscriptMetadata(chain.toString(), token, tokenId);
  } catch (e) {
    return {
      error: true,
