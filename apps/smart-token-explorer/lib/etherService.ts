...
export async function allowanceERC20(ERC20: string, owner: string, amount: string) {
    let balance = 0;
    try {
...
export async function validateToken(chain: number, owner: `0x${string}`, type: string, token: string, tokenId: string,) {
    ...
    if (result) {
        return { error: true, message: `This token on ${chainPipe(chain)} is not a valid ERC5169 token.` };
    }
    ...
    switch (type) {
        case 'ERC20': {
            ...
            if (!result) {
                return { error: true, message: `This token on ${chainPipe(chain)} is not a valid ERC20 token.` }
            }
        ...
        case 'ERC721': {
            ...
            if (!result) {
                return { error: true, message: `This token on ${chainPipe(chain)} is not a valid ERC721 token.` }
            }

            const allowance = await allowanceERC721(token, tokenId, owner);
            if (!allowance) {
                return { error: true, message: "You aren't not the owner of this token." }
            }
        ...
        default:   //1155
            {
                ...
                if (!result) {
                    return { error: true, message: `This token on ${chainPipe(chain)} is not a valid ERC1155 token.` }
                }
