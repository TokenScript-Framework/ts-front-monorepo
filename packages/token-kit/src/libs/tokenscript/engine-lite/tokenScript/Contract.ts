...
export class Contract {
  ...
  constructor(private contractDef: Element) {
    ...
    for (let i in addressesXml) {
      if (!addressesXml.hasOwnProperty(i)) continue;
  ...
  public getAbi(type?: string, name?: string) {
    if (!type && !name) return this.abi;

    let items = this.abi;

    if (type) items = items.filter((item: any) => type === item.type);

    if (name) items = items.filter((item: any) => name === item.name);
  ...
  public getFirstAddress() {
    ...
    return this.addresses[Number(keys[0])];
  }
  ...
  public getAddressByChain(
    ...
    if (!this.addresses[chain]) {
      ...
      throw new Error(
        "Contract does not have an address with chain ID " + chain,
      );
    }
